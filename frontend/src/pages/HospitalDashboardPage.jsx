...
const HospitalDashboardPage = () => {
  ...
  // ── Feature 14 states ──
  const [waitForm, setWaitForm]       = useState({ currentWait: '', hasER: false, erSpecialties: '' });
  ...
  const fetchHospital = async () => {
    ...
    try {
      ...
      // Pre-fill wait form if data exists
      if (data.erWaitTime) {
        setWaitForm({
          currentWait:   data.erWaitTime.currentWait || '',
          hasER:         data.erWaitTime.hasER || false,
          erSpecialties: (data.erWaitTime.erSpecialties || []).join(', '),
        });
      }
    } catch {
      setHospital(null);
  ...
  // ── Feature 14 handler ──
  const handleWaitUpdate = async (e) => {
    e.preventDefault();
    setWaitLoading(true);
    setWaitMsg('');
    try {
      await axios.put(
        '/api/hospitals/waittime/update',
        {
          currentWait:   Number(waitForm.currentWait) || 0,
          hasER:         waitForm.hasER,
          erSpecialties: waitForm.erSpecialties.split(',').map((s) => s.trim()).filter(Boolean),
        },
        { headers: { Authorization: 'Bearer ' + user.token } }
      );
      setWaitMsg('✅ Wait time updated!');
      setTimeout(() => setWaitMsg(''), 3000);
    } catch (err) {
      setWaitMsg('❌ ' + (err.response?.data?.message || 'Failed'));
    } finally {
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <div style={styles.container}>
                    ...
                    <div key={key} style={styles.capacityCell}>
                      ...
                      <div style={styles.capacityInputRow}>
                        <div style={styles.capacityInputGroup}>
                          ...
                          <input style={styles.capacityInput} type="number" min="0" max="9999"
                            value={val.total}
                            onChange={(e) => setCapacityEdit({
                              ...capacityEdit, [key]: { ...val, total: e.target.value },
                            })} />
                        </div>
                        ...
                        <div style={styles.capacityInputGroup}>
                          ...
                          <input
                            ...
                            value={val.available}
                            onChange={(e) => setCapacityEdit({
                              ...capacityEdit, [key]: { ...val, available: e.target.value },
                            })} />
                        </div>
                      </div>
                      {Number(val.total) > 0 && (
                        <>
                          <div style={styles.capacityBar}>
                            <div style={{ ...styles.capacityBarFill, width: `${pct}%`, backgroundColor: c.color }} />
                          </div>
                          <div style={{ ...styles.capacityPct, color: c.color }}>{pct}% available</div>
                        </>
                      )}
                    </div>
                  );
          ...
          <div style={styles.card}>
            ...
            <form onSubmit={handleInfoUpdate}>
              ...
              <div style={styles.fieldGroup}>
                ...
                <input style={styles.input} type="tel" placeholder="Emergency hotline"
        ...
        {activeTab === 'waittime' && (
          <div style={styles.card}>
            <h2 style={styles.cardTitle}>🚨 Update ER Wait Time</h2>
            <p style={styles.cardSub}>
              Update your current emergency department wait time so patients can compare nearby ERs.
            </p>
            {waitMsg && (
              <div style={waitMsg.includes('✅') ? styles.successMsg : styles.errorMsg}>
                {waitMsg}
              </div>
            )}
            <form onSubmit={handleWaitUpdate}>
              <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '16px' }}>
                <div style={styles.fieldGroup}>
                  <label style={styles.label}>Current Wait Time (minutes)</label>
                  <input style={styles.input} type="number" min="0"
                    placeholder="e.g. 45"
                    value={waitForm.currentWait}
                    onChange={(e) => setWaitForm({ ...waitForm, currentWait: e.target.value })} />
                </div>
                <div style={{ ...styles.fieldGroup, flex: 1, minWidth: '200px' }}>
                  <label style={styles.label}>ER Specialties (comma separated)</label>
                  <input style={styles.input} type="text"
                    placeholder="e.g. Trauma, Burns, Pediatric"
                    value={waitForm.erSpecialties}
                    onChange={(e) => setWaitForm({ ...waitForm, erSpecialties: e.target.value })} />
                </div>
              </div>
              <div style={styles.checkRow}>
                <input type="checkbox" id="hasER" checked={waitForm.hasER}
                  onChange={(e) => setWaitForm({ ...waitForm, hasER: e.target.checked })} />
                <label htmlFor="hasER" style={styles.checkLabel}>
                  This hospital has an active Emergency Room (ER)
                </label>
              </div>
              <button style={styles.saveBtn} type="submit" disabled={waitLoading}>
                {waitLoading ? 'Updating...' : '🚨 Update Wait Time'}
              </button>
            </form>
          </div>
        )}
...
const styles = {
  ...
  headerMeta:  { display: 'flex', justifyContent: 'center', gap: '12px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '20px' },
  typeBadge:   { backgroundColor: '#C6EBC5', color: '#27AE60', borderRadius: '12px', padding: '3px 12px', fontSize: '12px', fontWeight: 'bold' },
  ...
  statCard:  { backgroundColor: '#fff', borderRadius: '12px', padding: '18px', textAlign: 'center', border: '1px solid #C6EBC5' },
  ...
  capacityGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' },
  capacityCell: { backgroundColor: '#FEFDEC', borderRadius: '12px', padding: '16px', border: '1px solid #C6EBC5' },
  ...
  capacityBar:      { height: '6px', backgroundColor: '#f0f0f0', borderRadius: '3px', overflow: 'hidden', marginBottom: '4px' },
  ...
  fieldGroup: { marginBottom: '16px', flex: 1, minWidth: '200px' },
  ...
  // ── Feature 14 ──
  checkRow:   { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' },
